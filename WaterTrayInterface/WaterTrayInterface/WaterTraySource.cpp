...
long double konv(long double** f, int** g, int nx, int ny, int i, int j, long double hx, long double hy)
{

    if (j < ny / 3)
    {

        long double ap, am, bp, bm, c, u, v;
        ...
        u = f[i][j];
        v = (f[i][j + ny / 3] + f[i][j + ny / 3 + 1] + f[i - 1][j + ny / 3] + f[i - 1][j + ny / 3 + 1]) / 4.0;


        return (u - fabsl(u)) * ap / 2.0 + (u + fabsl(u)) * am / 2.0 + (v - fabsl(v)) * bp / 2.0 + (v + fabsl(v)) * bm / 2.0;
    }
    else if (j < 2 * ny / 3)
    {
        ...
        u = (f[i][j - ny / 3] + f[i + 1][j - ny / 3] + f[i][j - ny / 3 - 1] + f[i + 1][j - ny / 3 - 1]) / 4.0;
        v = f[i][j];
...
long double lapl(long double** f, int** g, int nx, int ny, int i, int j, long double hx, long double hy)
{
    ...
    if (j < ny / 3)
    {
        ...
        a = (f[i + 1][j] - 2.0 * f[i][j] + f[i - 1][j]) / (hx * hx);
        b = (f[i][j + 1] - 2.0 * f[i][j] + f[i][j - 1]) / (hy * hy);
...
long double dif_visc(long double** f, int** g, int nx, int ny, int i, int j, long double hx, long double hy, long double** Mu, long double** Ro)
{
    ...
    if (j < ny / 3)
    {
        ...
        a = 2.0 * (Mu[i][j] * (f[i + 1][j] - f[i][j]) - Mu[i - 1][j] * (f[i][j] - f[i - 1][j])) / (hx * hx);
        b = ((Mu[i][j] + Mu[i - 1][j] + Mu[i][j + 1] + Mu[i - 1][j + 1]) * ((f[i][j + ny / 3 + 1] - f[i - 1][j + ny / 3 + 1]) / hx + (f[i][j + 1] - f[i][j]) / hy) / 4 -
            (Mu[i][j] + Mu[i - 1][j] + Mu[i][j - 1] + Mu[i - 1][j - 1]) * ((f[i][j + ny / 3] - f[i - 1][j + ny / 3]) / hx + (f[i][j] - f[i][j - 1]) / hy) / 4) / hy;


        return 2.0 * (a + b) / (Ro[i][j] + Ro[i - 1][j]);
    }
    else if (j < 2 * ny / 3)
    {
        ...
        a = ((Mu[i][j - ny / 3] + Mu[i + 1][j - ny / 3] + Mu[i][j - ny / 3 - 1] + Mu[i + 1][j - ny / 3 - 1]) * ((f[i + 1][j - ny / 3] - f[i + 1][j - ny / 3 - 1]) / hy + (f[i + 1][j] - f[i][j]) / hx) / 4 -
            (Mu[i][j - ny / 3] + Mu[i - 1][j - ny / 3] + Mu[i][j - ny / 3 - 1] + Mu[i - 1][j - ny / 3 - 1]) * ((f[i][j - ny / 3] - f[i][j - ny / 3 - 1]) / hy + (f[i][j] - f[i - 1][j]) / hx) / 4) / hx;
        b = 2.0 * (Mu[i][j - ny / 3] * (f[i][j + 1] - f[i][j]) - Mu[i][j - ny / 3 - 1] * (f[i][j] - f[i][j - 1])) / (hy * hy);

        return 2.0 * (a + b) / (Ro[i][j - ny / 3] + Ro[i][j - ny / 3 - 1]);
    }
    else
    {
...
long double dob_lapl(long double** f, long double** Ro, int** g, int nx, int ny, int i, int j, long double hx, long double hy)
{
    ...
    if (j < ny / 3)
    {
        ...
        a = (f[i + 1][j] - f[i - 1][j]) / (2.0 * hx);
        b = (f[i][j + 1] - f[i][j - 1]) / (2.0 * hy);

        c = (Ro[i + 1][j] - Ro[i - 1][j]) / (2.0 * hx);
        d = (Ro[i][j + 1] - Ro[i][j - 1]) / (2.0 * hy);
    ...
    else
    {
        ...
        c = (Ro[i + 1][j - 2 * ny / 3] - Ro[i - 1][j - 2 * ny / 3]) / (2.0 * hx);
        d = (Ro[i][j - 2 * ny / 3 + 1] - Ro[i][j - 2 * ny / 3 - 1]) / (2.0 * hy);
...
int main()
{
    int nx = 300, ny = 108, i, j, p, T = 4001, t;
    int nx1 = 89, nx2 = 93, ny1 = 14, ny2 = 28, ny3 = 68;
    long double ax = 0.0, bx = 6.0, ay = 0.0, by = 2.16;
    long double** un;
    long double** un1;
    long double** un2;
    long double** unpred;
    int** g;
    long double** rn;
    long double hx = (bx - ax) / nx, hy = (by - ay) / ny,
        eps = 1e-4, nrn, nrn0, ro1 = 1.2056, ro2 = 998.23, mu1 = 1.82 * 1e-5, mu2 = 1.004 * 1e-3, tau, pn12, D = 1e-10;    ///// mu1,ro1 - æèäêîñòü, mu2,ro2 - ïðèìåñü
    ...
    long double ROb, Pa, Pb, Vvoz, Vin, Mass, Mdelta, Sq = 0.013;
...
////ñòåíêè
    ...
    FILE* ff;
    ff = fopen("concetr2dnest120.dat", "w");
    fprintf(ff, "TITLE = Test\n");
    fprintf(ff, "VARIABLES = X,Y,U,V,P,C\n");
    ...
    ff = fopen("concetr2dnest120.dat", "a");
    ...
    while (t < T)
    {
        ...
        for (i = 1; i < nx; i++)
            for (j = 1; j < ny; j++)
            {
                ...
                if (g[i][j] == 1)
                {
                    ...
                    unpred[i][j] = un[i][j] + tau * (
                        -(un[i][j] - fabsl(un[i][j])) * (un2[i + 1][j] - un2[i][j]) / (2.0 * hx) - (un[i][j] + fabsl(un[i][j])) * (un2[i][j] - un2[i - 1][j]) / (2.0 * hx) - (v - fabsl(v)) * (un2[i][j + 1] - un2[i][j]) / (2.0 * hy) - (v + fabsl(v)) * (un2[i][j] - un2[i][j - 1]) / (2.0 * hy)
                        + 2.0 * (-D * dob_laplRo(Ro, nx, 3 * ny, i, j, hx, hy) * un2[i][j] + 2.0 * (Mu[i][j] * (un2[i + 1][j] - un2[i][j]) - Mu[i - 1][j] * (un2[i][j] - un2[i - 1][j])) / (hx * hx) + (mup * (un2[i][j + 1] - un2[i][j]) - mum * (un2[i][j] - un2[i][j - 1])) / (hy * hy) + (mup * (un2[i][j + ny + 1] - un2[i - 1][j + ny + 1]) - mum * (un2[i][j + ny] - un2[i - 1][j + ny])) / (hx * hy)
                            ) / (Ro[i][j] + Ro[i - 1][j]));
                ...
                if (g[i][j + ny] == 1)
                {
                    ...
                    unpred[i][j + ny] = un[i][j + ny] + tau * (
                        -(u - fabsl(u)) * (un2[i + 1][j + ny] - un2[i][j + ny]) / (2.0 * hx) - (u + fabsl(u)) * (un2[i][j + ny] - un2[i - 1][j + ny]) / (2.0 * hx) - (un[i][j + ny] - fabsl(un[i][j + ny])) * (un2[i][j + ny + 1] - un2[i][j + ny]) / (2.0 * hy) - (un[i][j + ny] + fabsl(un[i][j + ny])) * (un2[i][j + ny] - un2[i][j + ny - 1]) / (2.0 * hy)
                        + 2.0 * (-D * dob_laplRo(Ro, nx, 3 * ny, i, j + ny, hx, hy) * un2[i][j + ny] + (mup * (un2[i + 1][j + ny] - un2[i][j + ny]) - mum * (un2[i][j + ny] - un2[i - 1][j + ny])) / (hx * hx) + 2.0 * (Mu[i][j] * (un2[i][j + ny + 1] - un2[i][j + ny]) - Mu[i][j - 1] * (un2[i][j + ny] - un2[i][j + ny - 1])) / (hy * hy) + (mup * (un2[i + 1][j] - un2[i + 1][j - 1]) - mum * (un2[i][j] - un2[i][j - 1])) / (hx * hy)
                            ) / (Ro[i][j] + Ro[i][j - 1]) + getf(j + ny, 3 * ny));
        ...
        //////////////////////////////////////////////////////////////////////////////////////
        //////////// êîíâåêöèÿ-äèôôóçèÿ, ìåòîä ïðåäèêòîð-êîððåêòîð, 2 ïîðÿäîê òî÷íîñòè ïî âðåìåíè
        ...
        for (i = 1; i < nx; i++)
            for (j = 1; j < ny; j++)
            {
                ...
                {
                    cstart = j - 1;
                    if (gcon[i - 1][j] != -2) con1[i][cstart] = con1[i][cstart + 1];
                    cend = -1;
                }
                if (gcon[i][j] != 1 && cend == -1)
                {
                    cend = j + 1;
                    if (gcon[i][j] != -2) con1[i][cend - 1] = con1[i][cend - 2];
                    cstart = -1;
                }
        ...
        for (j = 1; j < ny; j++)
            for (i = 1; i < nx; i++)
            {
                ...
                {
                    cstart = i - 1;
                    if (gcon[i - 1][j] != -2) con2[cstart][j] = con2[cstart + 1][j];
                    cend = -1;
                }
                if (gcon[i][j] != 1 && cend == -1)
                {
                    cend = i + 1;
                    if (gcon[i][j] != -2) con2[cend - 1][j] = con2[cend - 2][j];
                    cstart = -1;
                }
        ...
        for (i = 1; i < nx; i++)
            for (j = 1; j < ny; j++)
                if (gcon[i][j] == 1)
                    con[i][j] = con[i][j] + tau * D * ((con2[i + 1][j] - 2.0 * con2[i][j] + con2[i - 1][j]) / (hx * hx) + (con2[i][j + 1] - 2.0 * con2[i][j] + con2[i][j - 1]) / (hy * hy))
                    - tau * ((con2[i + 1][j] - con2[i][j]) * (un[i][j] + un[i + 1][j] - fabsl(un[i][j] + un[i + 1][j])) / (4.0 * hx) + (con2[i][j] - con2[i - 1][j]) * (un[i][j] + un[i + 1][j] + fabsl(un[i][j] + un[i + 1][j])) / (4.0 * hx)
                        + (con2[i][j + 1] - con2[i][j]) * (un[i][j + ny] + un[i][j + ny + 1] - fabsl(un[i][j + ny] + un[i][j + ny + 1])) / (4.0 * hy) + (con2[i][j] - con2[i][j - 1]) * (un[i][j + ny] + un[i][j + ny + 1] + fabsl(un[i][j + ny] + un[i][j + ny + 1])) / (4.0 * hy));
        ...
        for (j = 1; j < ny; j++)
            for (i = 1; i < nx; i++)
            {
                ...
                {
                    cstart = i - 1;
                    if (gcon[i - 1][j] != -2) con[cstart][j] = con[cstart + 1][j];
                    cend = -1;
                }
                if (gcon[i][j] != 1 && cend == -1)
                {
                    cend = i + 1;
                    if (gcon[i][j] != -2) con[cend - 1][j] = con[cend - 2][j];
                    cstart = -1;
                }
        ...
        for (i = 1; i < nx; i++)
            for (j = 1; j < ny; j++)
            {
                ...
                {
                    cstart = j - 1;
                    if (gcon[i - 1][j] != -2) con[i][cstart] = con[i][cstart + 1];
                    cend = -1;
                }
                if (gcon[i][j] != 1 && cend == -1)
                {
                    cend = j + 1;
                    if (gcon[i][j] != -2) con[i][cend - 1] = con[i][cend - 2];
                    cstart = -1;
                }
